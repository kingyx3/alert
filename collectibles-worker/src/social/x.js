...
export const xAdapter = {
  ...
  key(account) { return account.username; },
  name(account) { return account.name || `@${account.username}`; },
  kind: "social",
  intervalSeconds(account) { return Math.max(60, Number(account.intervalSeconds || 180)); },
  fetchPosts,
  nextCursor(posts, priorCursor = null) { return posts.map((p) => p.id).sort().pop() || priorCursor || null; },
};
